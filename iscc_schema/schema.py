...
class IsccDeclaration(BaseModel):
    ...
    credentials: Optional[Union[str, Dict[str, Any], List]] = Field(
        ...
        description=(
            ...
            " value according to the following rules <ul> <li>If the value is a string interpret it"
            " as an URI. The expectation is that the URI returns a response with Content-type"
            " `application/json` where the data is a VC or an array of VCs.</li><li>If the value is"
            " an object interpret it as a VC according to the [Verifiable Credentials JSON"
            " Schema](https://w3c-ccg.github.io/vc-json-schemas/).</li><li>If the value is an array"
            " and an item in the array is an object interpret it as a VC.</li><li>If the value is"
            " an array and an items is a string interpret the item as an URI to"
            " VC(s)</li><li>Credentials are only taken into account if the"
            " [`credentialSubject`](https://www.w3.org/2018/credentials/#property-definitions) can"
            " be matched to the declarer (e.g."
