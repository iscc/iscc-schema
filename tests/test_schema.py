...
def test_validate_uri_ipfs_v2():
    assert IsccMetaV2(license="ipfs://some")
...
def test_jcs_big_int_raises_v2():
    obj = IsccMetaV2(duration=2**60)
    assert obj.dict() == {"duration": 1152921504606846976}
    with pytest.raises(ValueError):
        obj.jcs()
...
def test_schema_v2():
    so = IsccMetaV2(iscc="ISCC:EIAGUJFCEY")
    assert so.dict(exclude_none=True, by_alias=True, exclude_unset=False) == {
        "@context": f"http://purl.org/iscc/context",
        "@type": "CreativeWork",
        "$schema": f"http://purl.org/iscc/schema",
...
def test_pydantic_model_full_iscc_v2():
    obj = IsccMetaV2(iscc="ISCC:KID6X6GUH5F5GAXO2AUKQLUQFCUC4LNBCROR3QEP26N2PEOYVTDO2OY")
    assert obj.iscc == "ISCC:KID6X6GUH5F5GAXO2AUKQLUQFCUC4LNBCROR3QEP26N2PEOYVTDO2OY"
...
def test_pydantic_model_iscc_to_short_raises_v2():
    with pytest.raises(ValueError):
        IsccMetaV2(iscc="ISCC:EIAGUJFCE")
...
def test_pydantic_model_iscc_to_long_raises_v2():
    with pytest.raises(ValueError):
        IsccMetaV2(
            iscc="ISCC:KAD7LOFDIKZG5M426IITP2XOZ2S6YR3C4YNQ25URPKITNUL2NXLHU3SKFW336BFNK6WQ6"
        )
...
def test_identifier_string_v2():
    assert IsccMetaV2(identifier="some-id").dict() == {"identifier": "some-id"}
...
def test_identifier_list_v2():
    assert IsccMetaV2(identifier=["some-id", "other-id"]).dict() == {
        "identifier": ["some-id", "other-id"]
    }
...
def test_forbid_extra_fields_v2():
